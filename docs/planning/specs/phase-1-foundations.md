...
- **Account model**: mail authentication is via real OpenBSD system accounts — `useradd` with the shell forced to `/sbin/nologin`, home holding the Maildir; Dovecot authenticates against them with BSD authentication (`passdb { driver = bsdauth }`, which calls `auth_userokay(3)`) and OpenSMTPD through BSD auth as well. OpenBSD has no PAM, and its dovecot package is built `--without-pam`, so `bsdauth` is the only system-auth path. The nologin shell blocks interactive sessions without affecting either, because auth consults passwd and not the shell field. This is the shell-less-user property — no standard account gets an interactive shell. The operator's own account is the deliberate exception: it doubles as the admin login, so it keeps an interactive shell alongside the Maildir, and `deploy-mail.sh` creates it that way. The consequence is that the operator's mail credentials also grant a shell, which is accepted for that one account and is exactly why standard-tier accounts stay `nologin`. Standard accounts are created with `add-user.sh`; the onboarding service that automates that is Phase 3, not here.
...
- **DNS**: `nsd` hidden primary (`provide-xfr:` authenticated by the `kyriakon-he` TSIG key, hmac-sha256, secret substituted at deploy time and never committed, so no source-IP allowlist is needed), HE free secondary via AXFR/NOTIFY, TCP/53 inbound allowed in `pf`. Record set: MX 10, SPF `v=spf1 mx -all`, DKIM at `mail._domainkey` (private key on-box, excluded from repo), DMARC `p=none` + `rua=` (tightened to `p=quarantine` on 2026-09-18 once every aggregate row passed), wildcard `*` A/AAAA (a record, not a cert). PTR set at Hetzner, not in the zone.
